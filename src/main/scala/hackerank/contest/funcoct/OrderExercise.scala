...
object OrderExercise {
  ...
  def buildGraph(arr: Array[Int], maxArrs: Array[((Int, Int), Int)]): (Graph, Array[(Int, (Int, Int), Int)]) = {
    val foo = new Array[ArrayBuffer[Int]](20000).map { _ => new ArrayBuffer[Int]() }
    ...
    maxArrIdx.foreach { a =>
      ...
      (t._1 to t._2).foreach { i =>
        val buf = foo{arr{i} + 10000}
        if (buf.size != 0) {
          buf.foreach { x => graph.addEdge(a._1, x) }
        }
        buf.append(a._1)
      }
    }
...
//    val heap = Heap(arr, (x: (Int, (Int, Int), Int), y: (Int, (Int, Int), Int)) => x._3 > y._3 || x._2._1 < y._2._1 || (x._2._2 - x._2._1) < (y._2._2 - y._2._1))
    val heap = Heap(arr, (x: (Int, (Int, Int), Int), y: (Int, (Int, Int), Int)) => (x, y) match {
      case (a, b) if (a._3 == b._3) => a._2._1 < b._2._1
      case (a, b) if (a._3 == b._3 && a._2._1 == a._2._1) => (a._2._2 - a._2._1) < (b._2._2 - b._2._1)
      case (a, b) => a._3 > b._3
    })
    val result = new ArrayBuffer[Int]()
    val removed = new mutable.HashSet[Int]()
    while (heap.size != 0) {
      val foo = heap.get().get
      if (!removed.contains(foo._1)) {
        result.append(foo._3)
        graph.adj(foo._1).foreach { x =>
          graph.removeV(x)
          removed.add(x)
        }
      }
...
//    val h = Heap.apply(Array(5, 15), (a: Int, b: Int) => a > b)
//    println(h.toString())
    // println(maxSum(List(-2, -4, 10, 4, -2, 3, -19, 2, 3), 0, new ArrayBuffer[((Int, Int), Int)](), ((0, 0), 0), ((0, 0), 0)).mkString(" "))
//    val r = new Random
//    val list = (1 to 100000).map { _ => r.nextInt(20000) - 10000 }.toList
//    println(list.mkString(" "))
//    println(disjointSets(List(-2, -4, 10, 4, -2, 3, -19, 2, 3)))
    val arr = Console.readLine().split(" ").map(_.toInt)
    val k = arr{1}
    val list = Console.readLine().split(" ").map(_.toInt).toList
    val maxSums = disjointSets(list)
    if (maxSums.size < k) println(maxSums.mkString("\n")) else maxSums.slice(0, k).mkString("\n")
    println(disjointSets(list))
  }
}
